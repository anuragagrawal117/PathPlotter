class GeocoderService
    ...
    Geokit::Geocoders::GoogleGeocoder.api_key = 'AIzaSyAb8ohmbxqtK4y2_a5CFnFnfLGiOsuwjIo'
	...
	def get_plotted_coordinates(interval_distance, remaining_distance, polyline)
    plotted_coordinates = []
    if remaining_distance > 0.0
      current_interval = remaining_distance
    else
      current_interval = interval_distance
    end
    decoded_coordinates = fetch_decoded_coordinates(polyline)
    distance_left = current_interval
    current_coordinate = decoded_coordinates.first

    decoded_coordinates[1..-1].each do |decoded_coordinate|
    	ll1 = Geokit::LatLng.new(current_coordinate.x, current_coordinate.y)
      ll2 = Geokit::LatLng.new(decoded_coordinate.x, decoded_coordinate.y)
      distance_between = ll1.distance_to(ll2)
      if(distance_between < distance_left)
        distance_left -= distance_between
      else
      	ll3 = latlong_after(ll1, ll2, distance_left)
      	coordinate = Coordinate.new(ll3.lat, ll3.lng)
      	plotted_coordinates << coordinate
      	distance_left = interval_distance - ll3.distance_to(ll2)
      end
      current_coordinate = decoded_coordinate
      remaining_distance = distance_left
    end
    return [plotted_coordinates, remaining_distance]
  end

  def fetch_decoded_coordinates(polyline)
    coordinates = []
    begin
      decoded_points = FastPolylines.decode(polyline)
    rescue Exception => e
      p "unable to decode #{polyline}, error: #{e.message}"
    end
    decoded_points.each do |point|
      coordinates << Coordinate.new(point.first, point.last)
    end
    return coordinates
  end

  def latlong_after(latlon1, latlon2, distance)
    latlon1.endpoint(latlon1.heading_to(latlon2), distance) 
  end
